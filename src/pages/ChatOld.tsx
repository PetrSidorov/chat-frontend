...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    socket.on("send-back", (data) => {
      console.log("load-messages emit from server: ", data);
      setDialog(
      ...
      console.log("dialog now: ", dialog);
      console.log("---------");
    });

    socket.on("send-messages-to-client", (data) => {
      console.log("data send mess to client", data);
      setDialog(
        data.data.messages.map((message) => {
          const user = users.find((user) => user.id === message.senderId);
  ...
  function loadChat(companionId, username) {
    // console.log("loadChat(companionId, username)", companionId, username);
    //websocket
